/* session.js - sessions module for node.js
   ...
   Additional API modiftications
    ...
    - secure
      ...
      A boolean flag to indicate that a cookie should only be sent when the request is https

    - httponly

      A boolean flag to indicate that the cookie should be httponly (not readable by javascript)
...
function lookupOrCreate(req, opts) {
  ...
  if (ownProp(sessions, id)) {
    session = sessions[id];
  } else { // otherwise create a new session object with that ID, and store it
...
Session.prototype.getSetCookieHeaderValue = function() {
  ...
  if (this.secure) parts.push('secure=' + this.secure);
  if (this.httponly) parts.push('httponly=' + this.httponly)
